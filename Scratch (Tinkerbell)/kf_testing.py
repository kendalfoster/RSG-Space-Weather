...
for i in range(0, num_st-1): # first station, initialize the DataArray
    ...
    coeffs_fs = coeffs_fs.assign_coords(first_st = st_1.station.values[0])
    coeffs_fs = coeffs_fs.assign_coords(second_st = st_2.station.values[0])
    coeffs_fs = coeffs_fs.squeeze(dim = ['dim_0', 'dim_1'], drop = True)

    # loop through the other stations ------------------------------------------
    # check if there are at least three stations (I think)
    if num_st >= 3:
        for j in range(i+2, num_st): # second station
            ...
            cfs = cfs.assign_coords(first_st = st_1.station.values[0])
            cfs = cfs.assign_coords(second_st = st_2.station.values[0])
